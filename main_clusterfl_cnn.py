...
class ClusterFLClient:
    ...
    def fine_tune(self, epochs):
        """
        Standard local fine-tuning (no proximal term) for personalization.
        """
        self.model.set_weights(self.weights)
        # Use standard compile/fit for pure local adaptation
...
# --- Main Run ---
def run_clusterfl(train_datasets, test_datasets, num_clients, num_rounds=50, 
                  alpha=0.1, seed=42, clients_per_round=10,fine_tune_epochs=0):
    ...
    if fine_tune_epochs > 0:
        print(f"\n[INFO] Fine-tuning ClusterFL clients for {fine_tune_epochs} epochs...")
        for cid, client in clients.items():
            client.fine_tune(epochs=fine_tune_epochs)
    # 4. Final Evaluation
    print("\nFinal Evaluation:")
    results = {}
    for cid, client in clients.items():
