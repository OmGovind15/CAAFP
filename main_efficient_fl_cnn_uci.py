...
def run_efficient_fl(num_clients=30, num_rounds=50, clients_per_round=10, 
                     target_sparsity=0.7, alpha=0.1, seed=42, fine_tune_epochs=3):
    ...
    for cid, client in clients.items():
        client.fine_tune(final_pruned_model, epochs=fine_tune_epochs)
        res = client.evaluate()
        results[cid] = res
    ...
    # DEBUG PRINT: Uncomment this if you still get 0 to see what is being returned
    # print(f"DEBUG: raw_metrics type: {type(raw_metrics)}, content keys: {raw_metrics.keys() if isinstance(raw_metrics, dict) else 'list'}")

    if raw_metrics:
        # Case A: Dictionary of rounds (e.g. {0: {...}, 1: {...}})
        if isinstance(raw_metrics, dict):
            # Check if it's already the flat metric dict (has 'total_comm_mb')
            if 'total_comm_mb' in raw_metrics:
                final_metrics_fl = raw_metrics
            # Otherwise, find the last round
            else:
                try:
                    # Filter keys to ensure they are integers (round numbers)
                    round_keys = [k for k in raw_metrics.keys() if isinstance(k, int)]
                    if round_keys:
                        last_round = max(round_keys)
                        final_metrics_fl = raw_metrics[last_round]
                except Exception as e:
                    print(f"Warning: Could not extract metrics from dict: {e}")

        # Case B: List of dicts (standard) -> Take last
        elif isinstance(raw_metrics, list):
            final_metrics_fl = raw_metrics[-1]
            
    # Force default if extraction failed but we have data
    if not final_metrics_fl and raw_metrics and isinstance(raw_metrics, dict):
         # Try grabbing *any* dictionary value that looks like metrics
         for v in raw_metrics.values():
             if isinstance(v, dict) and 'total_comm_mb' in v:
                 final_metrics_fl = v
                 break
