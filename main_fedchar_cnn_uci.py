...
class FedCHARServer:
    ...
    def cluster_clients(self, client_updates):
        ...
        for cluster_id in range(self.num_clusters):
            self.cluster_models[cluster_id] = create_cnn_model(input_shape=(128, 9))
...
class FedCHARClient:
    def __init__(self, client_id, train_dataset, test_dataset, lambda_reg=0.01):
        ...
        self.personal_model = create_cnn_model(input_shape=(128, 9))
...
def run_fedchar(num_clients=30, num_rounds=40, initial_rounds=10,alpha=0.1,
                clients_per_round=10, epochs_per_round=5, seed=42,fine_tune_epochs=3):
    ...
    for cid, client in clients.items():
        cluster_w = server.get_model_for_client(cid)
        client.fine_tune(cluster_w, epochs=fine_tune_epochs)
        res = client.evaluate()
        results[cid] = res
