...
class CAAFPServer:
    ...
    def update_masks_and_evolve(self, cluster_id, client_models_in_cluster, client_gradients_in_cluster, 
                               ...
                               pruning_steps_remaining,  # <--- NEW ARGUMENT
        ...
        for idx, layer in enumerate(prunable_layers):
            ...
            # CASE B: EVOLUTION (Maintenance + Progressive Push)
                ...
                # --- NEW LOGIC: Use passed argument ---
                steps_left = max(1, pruning_steps_remaining)
                
                sparsity_gap = target_sparsity - local_sparsity
                n_deficit_push = int(np.round(sparsity_gap * n_total / steps_left))
                ...
                if idx == 0:
                    print(f"    [LOGIC] Evolution. Steps Left: {steps_left}. Deficit: {n_deficit_push}, Churn: {n_churn}.")
...
def run_caafp(num_clients=30, num_rounds=50, initial_rounds=0,
              num_clusters=3,         # <--- ADDED THIS ARGUMENT (Default 3)
              post_cluster_warmup=0,  
    ...
    # UPDATE: Force num_clients to match the actual number of users found
    actual_num_clients = len(client_data)
    if num_clients != actual_num_clients:
        print(f"[INFO] Natural Split found {actual_num_clients} unique users. "
              f"Adjusting num_clients from {num_clients} to {actual_num_clients}.")
    ...
    # This line now works because num_clusters is passed in the arguments above
    server = CAAFPServer(num_clients=num_clients, num_clusters=num_clusters) 
    ...
    # --- PHASE 3: Pruning & Cluster Training ---
    print(f"\n{'='*60}")
    ...
    for round_num in range(initial_rounds, initial_rounds + total_pruning_rounds):
        ...
        # --- NEW LOGIC: Dynamic Pruning Check ---
        rounds_since_clustering = round_num - initial_rounds
        
        # Check if we are past the post-cluster warmup
        is_past_warmup = rounds_since_clustering >= post_cluster_warmup
        
        # Check if it is a pruning interval (relative to end of post-warmup)
        is_pruning_interval = (rounds_since_clustering - post_cluster_warmup) % pruning_frequency == 0
        
        is_pruning_round = is_past_warmup and is_pruning_interval
        
        # Calculate steps remaining for the Server function
        total_pruning_phase_rounds = clustering_training_rounds - post_cluster_warmup
        pruning_rounds_elapsed = max(0, rounds_since_clustering - post_cluster_warmup)
        
        total_steps = total_pruning_phase_rounds // pruning_frequency
        steps_elapsed = pruning_rounds_elapsed // pruning_frequency
        steps_remaining = max(1, total_steps - steps_elapsed)
        # ----------------------------------------
        ...
        for cluster_id in range(server.num_clusters):
            ...
            if is_pruning_round:
                ...
                server.update_masks_and_evolve(
                    ...
                    curr_sparsity, target_s, 
                    pruning_steps_remaining=steps_remaining,  # <--- PASS IT HERE
