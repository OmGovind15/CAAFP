...
# --- Main Run ---
def run_efficient_fl(train_datasets, test_datasets, num_clients, num_rounds=50, 
                     clients_per_round=10, target_sparsity=0.7, alpha=0.1, seed=42,
                     fine_tune_epochs=3):
    ...
    # Logic to find the data for the LAST round
    if raw_metrics:
        if isinstance(raw_metrics, list):
            # If it's a list of rounds, take the last one
            final_metrics_fl = raw_metrics[-1]
        elif isinstance(raw_metrics, dict):
            # If it's a dict keyed by round number, find the max key
            round_keys = [k for k in raw_metrics.keys() if isinstance(k, (int, float))]
            if round_keys:
                last_round = max(round_keys)
                final_metrics_fl = raw_metrics[last_round]
            else:
                # Fallback: maybe it is flat?
    ...
    rich_metrics = {
        ...
        'avg_sparsity': get_model_sparsity(final_pruned_model) if final_pruned_model else 0,

        # Now these will correctly pull from the extracted last_round dict
